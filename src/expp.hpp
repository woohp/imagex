...
struct function_traits<R (*)(Args...) noexcept>
{
    ...
    template <std::size_t... I>
    static R apply_impl(func_type fn, ErlNifEnv* env, const ERL_NIF_TERM argv[], std::index_sequence<I...>) noexcept
    {
        return fn(type_cast<std::decay_t<Args>>::load(env, argv[I])...);
    }

    static R apply(func_type fn, ErlNifEnv* env, const ERL_NIF_TERM argv[]) noexcept
    {
        return apply_impl(fn, env, argv, std::make_index_sequence<nargs> {});
    }
};
...
#define ELIXIR_MODULE(name, ...)                                                                                       \
    ErlNifFunc _nif_funcs[] = { __VA_ARGS__ };                                                                         \
    ERL_NIF_INIT(Elixir.name, _nif_funcs, nullptr, nullptr, nullptr, nullptr)
